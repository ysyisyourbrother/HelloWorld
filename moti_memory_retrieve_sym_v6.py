...
from src.system.symphony.v6.motivation import SymphonySystemMotiV6


def parse_args():
    parser = argparse.ArgumentParser(description="Symphony v6 单视频 agentic 检索作答")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/symconfig_v5_moti.json",
        help="配置文件路径（v6 可复用 v5 配置，建议将 benchmark.db_dir_* 指向 symphony_v6）",
    )
    parser.add_argument(
        "--video_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="检索对话 ID（可用于 clip 输出目录隔离）",
    )
    parser.add_argument(
        "-o",
        "--output_path",
        type=str,
        default="motivation_results_symphony_v6/retrieve",
        help="输出路径",
    )
    parser.add_argument(
        "--system_mode",
...
if __name__ == "__main__":
    ...
    moti = SymphonySystemMotiV6(config)
