...
def parse_args():
    parser = argparse.ArgumentParser(description="Symphony v4 检索并绘制相似度折线图")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/symconfig_v4_moti.json",
        help="配置文件路径",
    )
    parser.add_argument(
        "--topk",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="检索对话 ID（v4 可用于 clip 输出目录隔离）",
    )
    parser.add_argument(
        "-o",
        "--output_path",
        type=str,
        default="motivation_results_symphony_v4/retrieve",
        help="输出路径",
    )
...
def _to_nonneg_int_or_none(value):
...
def _load_frames_and_i_frames(map_path):
    ...
    raw_frames = video_obj.get("frames") or []
    raw_i_frames = video_obj.get("i_frames") or []

    frames = []
    for item in raw_frames:
        v = _to_nonneg_int_or_none(item)
        if v is not None:
            frames.append(v)

    i_frames = []
    for item in raw_i_frames:
        v = _to_nonneg_int_or_none(item)
        if v is not None:
            i_frames.append(v)
    i_frames = sorted(set(i_frames))

    total_frames = _to_nonneg_int_or_none(video_obj.get("total_frames"))
...
if __name__ == "__main__":
    ...
    moti = SymphonySystemMotiV4(config)
    ...
    if i_frame_ids:
        ...
        if len(gop_edges) >= 2:
            ...
            print("i_frames 不足以构成 GOP 区间，跳过 GOP 区间着色")
    else:
    ...
    if query_results:
        ...
        gathered_subtitles = first_query.get("gathered_subtitles")
        if gathered_subtitles:
            print("v4 字幕增强片段: {}".format(gathered_subtitles))
