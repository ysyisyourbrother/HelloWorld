#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Symphony v6 基准测试启动（MemoryAgent agentic 检索、is_local_vlm 等见 configs/symconfig_v6_moti.json）。"""
...
from src.system.symphony.v6.benchmark import SymphonySystemBenchV6


def parse_args():
    parser = argparse.ArgumentParser(description="SymphonySystemBenchV6 云边集成 Benchmark")
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/symconfig_v6.json",
        help="配置文件路径（默认 configs/symconfig_v6.json）",
    )
...
if __name__ == "__main__":
    ...
    # v6 编排与 v5 一致：GOP 扫描走 ffprobe（V1），避免误走 GStreamer 路径。
    ...
    bench = SymphonySystemBenchV6(config)
    bench.run(
