...
def _load_bge_patch_params(config_path: str) -> tuple[int, int, int]:
    """从 BGE 模型配置中读取 image_size、patch_size 和 num_hidden_layers"""
...
def get_attention_map(
    ...
    """
    提取 BGE ViT 中 softmax(QK^T) 的 Attention Map。
    取 CLS token（query pos 0）对各 patch（key pos 1~196）的注意力权重，多头平均后 reshape 为 patch 网格。

    Args:
        layer_idx: 使用哪一层的 attention，-1 表示最后一层。
    ...
    model = CLIPModel.from_pretrained(
        model_path,
        attn_implementation="eager",  # 确保支持 output_attentions
    ).to(device)
    ...
    # attentions: tuple of (bsz, num_heads, tgt_len, src_len) 每层一个
    all_attentions = vision_outputs.attentions
    attn = all_attentions[layer_idx]  # [1, num_heads, 197, 197]
    # CLS (query 0) -> patches (key 1~196)
    cls_to_patch = attn[:, :, 0, 1:].cpu().numpy()  # [1, num_heads, 196]
    scores_flat = cls_to_patch.mean(axis=1).squeeze(0)  # [196]

    num_per_side = image_size // patch_size
    scores = scores_flat.reshape(num_per_side, num_per_side)
    return scores
...
def visualize_cls_patch_scores_on_original(
    ...
    print(f"Attention Map 热力图已保存到: {output_path}")
...
def visualize_cls_patch_scores_with_labels(
    ...
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
    cv2.imwrite(output_path, canvas)
    print(f"Attention Map 热力图（含分数标注）已保存到: {output_path}")
    print(f"  - 分数范围: [{s_min:.4f}, {s_max:.4f}]")
...
def visualize_cls_patch_scores_overlay(
    ...
    print(f"Attention Map 叠加图已保存到: {output_path} (alpha={alpha})")
...
def main():
    parser = argparse.ArgumentParser(description="BGE patch 网格与 Attention Map (softmax(QK^T)) 可视化")
    ...
    parser.add_argument("--no_patch_grid", action="store_true", help="不生成 patch 网格图，仅生成 Attention Map 热力图")
    parser.add_argument("--layer", type=int, default=-1, help="使用哪一层的 attention，-1 表示最后一层")
    ...
    isz, psz, num_layers = _load_bge_patch_params(args.config)
    image_size = args.image_size if args.image_size is not None else isz
    patch_size = args.patch_size if args.patch_size is not None else psz
    layer_display = args.layer if args.layer >= 0 else num_layers + args.layer  # -1 -> 最后一层的实际索引
    device = args.device or config.frame_device
    model_path = config.frame_model_path

    # 所有输出路径基于输入路径 -i 自动生成，_l0 表示第 0 层
    base, ext = os.path.splitext(args.input)
    layer_suffix = f"_l{layer_display}"
    output = f"{base}_patch_grid.jpg"
    output_original = f"{base}_patch_grid_original_size.jpg"
    output_attn = f"{base}_attn_weight{layer_suffix}.jpg"
    output_overlay = f"{base}_attn_overlay{layer_suffix}.jpg"
    output_labels = f"{base}_attn_labels{layer_suffix}.jpg"
